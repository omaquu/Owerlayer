...
pub enum BrushMode { Solid, Highlighter, Calligraphy }
...
#[derive(Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub enum StrokeKind { Freehand, Line, Arrow, Rect, Circle, Star, Heart }
...
#[derive(Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub enum EraserMode { Stroke, Split }
...
fn tool_btn(ui: &mut egui::Ui, label: &str, active: bool, tooltip: &str, accent: egui::Color32) -> bool {
    let size = egui::vec2(28.0, 28.0);
    let fill = if active { 
        egui::Color32::from_rgba_premultiplied(accent.r(), accent.g(), accent.b(), 180)
    } else if accent != egui::Color32::from_gray(150) {
        // If a special color was passed (like red for X), use it as base
        egui::Color32::from_rgba_premultiplied(accent.r(), accent.g(), accent.b(), 150)
    } else { 
        egui::Color32::from_rgb(60, 60, 60) 
    };
    let text_col = if active { egui::Color32::WHITE } else { egui::Color32::from_gray(200) };
    let mut btn = egui::Button::new(egui::RichText::new(label).size(14.0).color(text_col))
        .fill(fill)
        .stroke(egui::Stroke::new(1.0, egui::Color32::from_rgb(40, 40, 40)))
        .corner_radius(egui::CornerRadius::same(4))
        .min_size(size);
        
    if accent != egui::Color32::from_gray(150) && !active {
        btn = btn.fill(egui::Color32::from_rgba_premultiplied(accent.r(), accent.g(), accent.b(), 100));
    }
...
pub fn render_photoshop_panel(
    ...
    let main_tools = vec![
        Tool::Move, Tool::Brush, Tool::Eraser, Tool::Text, Tool::Shape, Tool::Snip, Tool::Cut,
    ];
    ...
    let mut win = egui::Window::new("photoshop_panel")
        ...
        .frame(frame);
    if is_vertical { win = win.max_width(40.0); }
    ...
    win.show(ctx, |ui| {
            ...
            if is_vertical {
                ui.vertical(|ui| {
                    ...
                    ui.separator();
                    if tool_btn(ui, "📑", *show_layers_panel, "Layers", accent) { *show_layers_panel = !*show_layers_panel; }
                    if tool_btn(ui, "⚙", *show_settings_panel, "Settings", accent) { *show_settings_panel = !*show_settings_panel; }
                    if tool_btn(ui, "×", false, "Close App", egui::Color32::from_rgb(200, 50, 50)) { *show_exit_dialog = true; }
                    ui.separator();
                    // Small options for vertical mode
                    let mut fg_c = color32(&settings.pen_color);
                    if ui.color_edit_button_srgba(&mut fg_c).on_hover_text("Pen Color").changed() { settings.pen_color = [fg_c.r(), fg_c.g(), fg_c.b(), fg_c.a()]; }
                    let mut bg_c = color32(&settings.background_color);
                    if ui.color_edit_button_srgba(&mut bg_c).on_hover_text("Fill Color").changed() { settings.background_color = [bg_c.r(), bg_c.g(), bg_c.b(), bg_c.a()]; }
                    ui.add(egui::Slider::new(&mut settings.pen_width, 1.0..=100.0).vertical().show_value(false));
                });
            } else {
                ui.vertical(|ui| {
                    // Top Row: Tools
                    ui.horizontal(|ui| {
                        for tool in &main_tools {
                            let is_selected = *active_tool == *tool;
                            let response = tool_btn_custom(ui, *tool, is_selected);
                            if response.clicked() { *active_tool = *tool; }
                        }
                        ui.separator();
                        if tool_btn(ui, "📑", *show_layers_panel, "Layers", accent) { *show_layers_panel = !*show_layers_panel; }
                        if tool_btn(ui, "⚙", *show_settings_panel, "Settings", accent) { *show_settings_panel = !*show_settings_panel; }
                    ...
                    ui.horizontal(|ui| {
                        ...
                        match active_tool {
                            Tool::Brush | Tool::Eraser | Tool::Shape => {
                                ui.label("Size:");
                                ui.add(egui::Slider::new(&mut settings.pen_width, 1.0..=100.0).show_value(false));
                                if *active_tool == Tool::Brush {
                                    ...
                                    ui.selectable_value(&mut settings.eraser_mode, EraserMode::Split, "Split");
                                } else {
                            ...
                            Tool::Text => {
                                ui.label("Size:");
                                ui.add(egui::Slider::new(&mut settings.font_size, 10.0..=100.0).show_value(false));
                                ui.checkbox(&mut settings.text_shadow, "Shadow");
                                ui.checkbox(&mut settings.text_outline, "Outline");
                            }
                            Tool::Snip => { ui.checkbox(&mut settings.snip_shadow, "Shadow"); }
                            Tool::Cut => {
                                ...
                                if settings.cut_mode == CutMode::MagicWand {
                                    ui.add(egui::Slider::new(&mut settings.magic_wand_threshold, 0.0..=100.0).show_value(true));
                                }
                            }
                            _ => { ui.label(egui::RichText::new(active_tool.name()).size(12.0).color(egui::Color32::from_gray(150))); }
                        }
                    });
                });
            }
    ...
    for (i, layer) in project.layers.iter_mut().enumerate().filter(|(_, l)| l.visible) {
        ...
        for s in layer.strokes.iter() {
            ...
            if s.brush_mode == BrushMode::Highlighter {
                c = egui::Color32::from_rgba_premultiplied(c.r() / 2, c.g() / 2, c.b() / 2, (80.0 * l_op) as u8);
            }
            match s.kind {
                StrokeKind::Freehand => {
                    if s.points.len() >= 2 {
        ...
        for ann in layer.text_annotations.iter() {
            let font = if ann.monospace { egui::FontId::monospace(ann.font_size) } else { egui::FontId::proportional(ann.font_size) };
    ...
    match active_tool {
        ...
        Tool::Snip => {
            ...
            if left_just_released {
                if let Some(start) = line_start.take() {
                    ...
                    let x = rect.min.x.max(0.0) as i32;
                    let y = rect.min.y.max(0.0) as i32;
                    let w = rect.width() as i32;
                    let h = rect.height() as i32;
                    if w > 5 && h > 5 {
                        if let Some(pixels) = crate::winapi_utils::capture_screen_rect(x, y, w, h) {
                            layer.placed_images.push(PlacedImage {
                                id: layer.placed_images.len(), position: rect.min,
                                size: [w as usize, h as usize], display_size: None,
        ...
        Tool::Move => {
            if let Some(bounds) = layer_bounds(layer) {
                ...
                ui.allocate_ui_at_rect(top_btns_rect, |ui| {
                    ui.horizontal(|ui| {
