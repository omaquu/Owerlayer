...
impl eframe::App for OwerlayerApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                overlay::render_canvas(ui, &self.active_tool, &mut self.settings,
                    &mut self.project, &mut self.current_stroke, &mut self.line_start,
                    &mut self.pending_text, &mut self.last_tool_used,
                    self.edit_mode, &mouse, can_draw);
            });

        // ---- 7. Repaint strategy ----
        if self.edit_mode {
            ctx.request_repaint(); // Native framerate for smooth brush
...
fn main() -> eframe::Result<()> {
    ...
    let icon_data = match image::load_from_memory(include_bytes!("../icon.png")) {
        Ok(img) => {
