    ...
    match active_tool {
        Tool::Brush | Tool::Eraser => {
            ...
            if *active_tool == Tool::Brush {
                ...
                if is_vertical {
                    if ui.add(egui::Button::new(if settings.brush_shadow { "★ Shadow ON" } else { "☆ Shadow OFF" }).min_size(egui::vec2(40.0, 24.0))).clicked() {
                        settings.brush_shadow = !settings.brush_shadow;
                    }
                } else {
        ...
        Tool::Text => {
            ...
            if is_vertical {
                if ui.add(egui::Button::new(if settings.text_shadow { "★ Shadow" } else { "☆ Shadow" })).clicked() {
                    settings.text_shadow = !settings.text_shadow;
                }
                if ui.add(egui::Button::new(if settings.text_outline { "⬌ Outline" } else { "⬔ Outline" })).clicked() {
        ...
        Tool::Snip => {
            ...
            if is_vertical {
                if ui.add(egui::Button::new(if settings.snip_shadow { "★ Shadow" } else { "☆ Shadow" })).clicked() {
        ...
        Tool::Blur => {
            ...
            if left_just_released {
                if let Some(start) = line_start.take() {
                    ...
                    let h = rect.height();
                    if w > 4.0 && h > 4.0 {
                        // Create Blur layer if not exists
                ...
                if left_just_pressed {
                    ...
                    if !hit {
                        for (idx, &mp) in mids.iter().enumerate() {
                            let handle_rect = egui::Rect::from_center_size(mp, egui::vec2(12.0, 12.0));
                ...
                if let Some(start) = *line_start {
                    if left_down {
                        ...
                        if start.x == -1.0 && start.y == -1.0 {
                            // Rotation
                            let center = initial_center.unwrap_or(bounds.center());
                            ...
                            rotate_layer(layer, center, angle_delta);
                        } else if start.x == -3.0 {
                            // Perspective
                            ...
                            let center = initial_center.unwrap_or(bounds.center());
                            let skew_delta = if handle_idx == 0 || handle_idx == 1 {
                                // Mid-left or mid-right handles skew vertically
                                egui::vec2(0.0, delta.y * 0.01)
                            } else {
                                // Mid-top or mid-bottom handles skew horizontally
                                egui::vec2(delta.x * 0.01, 0.0)
                            };
                            skew_layer(layer, center, skew_delta);
                        } else if start.y == -1.0 {
                            // Resizing
                            let handle_idx = start.x as usize;
                            let ib = initial_bounds.unwrap_or(bounds);
                            let ic = [ib.left_top(), ib.right_top(), ib.left_bottom(), ib.right_bottom()];
                            let anchor = match handle_idx {
                                0 => ic[3], 1 => ic[2], 2 => ic[1], 3 => ic[0], _ => ic[0],
                            };
