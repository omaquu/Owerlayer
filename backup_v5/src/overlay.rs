    ...
    win.show(ctx, |ui| {
            ...
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    if ui.add(egui::Button::new("📁").min_size(egui::vec2(28.0, 24.0))).clicked() { *show_layers_panel = !*show_layers_panel; }
    ...
    match active_tool {
        Tool::Brush | Tool::Eraser => {
            ...
            if *active_tool == Tool::Brush {
                ...
                if is_vertical {
                    let btn_bg = if settings.brush_shadow { egui::Color32::from_rgb(60, 120, 200) } else { egui::Color32::from_rgb(50, 50, 50) };
                    let btn = ui.add(egui::Button::new("").min_size(egui::vec2(28.0, 24.0)).fill(btn_bg));
                    let rect = btn.rect;
                    
                    // Draw the permanent fake shadow behind "S"
                    ui.painter().text(rect.center() + egui::vec2(1.0, 1.0), egui::Align2::CENTER_CENTER, "S", egui::FontId::proportional(16.0), egui::Color32::from_gray(80));
                    
                    if btn.on_hover_text("Brush Shadow").clicked() { settings.brush_shadow = !settings.brush_shadow; }
                    
                    let s_color = if settings.brush_shadow { egui::Color32::WHITE } else { egui::Color32::from_gray(200) };
                    ui.painter().text(rect.center(), egui::Align2::CENTER_CENTER, "S", egui::FontId::proportional(16.0), s_color);
                    
                    ui.add_space(2.0);
                    
                    // Outline (Stroke) button
                    let o_bg = if settings.brush_outline { egui::Color32::from_rgb(60, 120, 200) } else { egui::Color32::from_rgb(50, 50, 50) };
                    let o_btn = ui.add(egui::Button::new("").min_size(egui::vec2(28.0, 24.0)).fill(o_bg));
                    let o_rect = o_btn.rect;
                    ui.painter().text(o_rect.center(), egui::Align2::CENTER_CENTER, "O", egui::FontId::proportional(16.0), egui::Color32::WHITE);
                    if o_btn.on_hover_text("Brush Stroke/Outline").clicked() { settings.brush_outline = !settings.brush_outline; }
                } else {
                    ui.toggle_value(&mut settings.brush_shadow, "Shadow");
                    ui.toggle_value(&mut settings.brush_outline, "Stroke");
                }
            } else {
                if is_vertical {
                    ui.vertical(|ui| {
        ...
        Tool::Shape => {
            ...
            ui.toggle_value(&mut settings.shape_fill, "Fill");
        }
        Tool::Snip => {
            if is_vertical {
                ui.vertical(|ui| {
        ...
        Tool::Cut => {
            ...
            ui.toggle_value(&mut settings.inverted_cut, "Invert");
...
pub fn render_canvas(
    ...
    for (i, layer) in project.layers.iter_mut().enumerate().filter(|(_, l)| l.visible) {
        ...
        for img in layer.placed_images.iter_mut() {
            if img.is_live {
                ...
                let (sx, sy, sw, sh) = if let Some(src) = img.source_rect {
                    (((src[0] * ppp) - 1.0) as i32, ((src[1] * ppp) - 1.0) as i32, (img.size[0] as f32) as i32, (img.size[1] as f32) as i32)
                } else {
                    ...
                    img.size = [(aabb.width() * ppp).ceil() as usize, (aabb.height() * ppp).ceil() as usize];
                    // sx/sy are screen pixels. egui point 0.0 is screen pixel -1.0.
                    // screen_pixel = (egui_point * ppp) - 1.0
                    (((aabb.min.x * ppp) - 1.0) as i32, ((aabb.min.y * ppp) - 1.0) as i32, img.size[0] as i32, img.size[1] as i32)
                };
        ...
        for s in layer.strokes.iter() {
                            ...
                            if s.brush_mode == BrushMode::Spray {
                                ...
                                for window in pts.windows(2) {
                                    ...
                                    if s.brush_shape == BrushShape::Square {
                                        ...
                                        for pt in &pts {
                                            p.circle_filled(*pt, s.width * 0.5, col);
                                        }
                                    }
                                }
                            }
            ...
            if layer.shadow || s.shadow {
                let s_col = if let Some(bg) = s.background_color {
                    let mut c = color32(&bg);
                ...
                draw_stroke(&painter, s_col, egui::vec2(2.0, 2.0));
            }
            if layer.outline || s.outline {
                let mut o_col = if c.r() as u32 + c.g() as u32 + c.b() as u32 > 382 { egui::Color32::BLACK } else { egui::Color32::WHITE };
                if let Some(bg) = s.background_color {
                    let bg_c = color32(&bg);
                    if bg_c.a() > 0 { o_col = egui::Color32::from_rgba_unmultiplied(bg_c.r(), bg_c.g(), bg_c.b(), (255.0 * l_op) as u8); }
                }
    ...
    if can_draw && edit_mode && matches!(active_tool, Tool::Move) {
        ...
        if left_just_pressed || is_double_click {
            // 1. Prioritize active layer handles/body
            let mut hit_active = false;
            if let Some(l) = project.get_active_layer() {
                if let Some(b) = layer_bounds(l) {
                    if b.expand(30.0).contains(pos) { hit_active = true; }
                }
            }

            if !hit_active || is_double_click {
    ...
    match active_tool {
        Tool::Brush => {
            ...
            if left_just_released && !current_stroke.is_empty() {
                    layer.strokes.push(Stroke {
                        ...
                        skew: egui::Vec2::ZERO,
                        outline: settings.brush_outline,
                    });
                    current_stroke.clear();
                }
            }
        ...
        Tool::Move => {
            if let Some(bounds) = layer_bounds(layer) {
                ...
                if ui.input(|i| i.pointer.button_double_clicked(egui::PointerButton::Primary)) && bounds.contains(pos) {
                    for img in &mut layer.placed_images {
                ...
                if left_just_pressed && !*dragging_source_rect {
                    ...
                    if !hit {
                        // Perspective handles (20-23)
                        for (idx, pc) in p_corners.iter().enumerate() {
